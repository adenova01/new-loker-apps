                    ...
                    <span class="pull-right">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            <a class="dropdown-item" href="javascript:editStatus(<?php echo e($column->id); ?>);">
                                <i class="fa fa-edit"></i> <?php echo app('translator')->get('app.edit'); ?>
                            </a>
                            <?php if($column->id > 5): ?>
                            <a class="dropdown-item" href="javascript:deleteStatus(<?php echo e($column->id); ?>);">
                                <i class="fa fa-trash-o"></i> <?php echo app('translator')->get('app.delete'); ?>
                            </a>
                            <?php endif; ?>
                        </div>
                    </span>
                                                ...
                                                <span id="buttonBox<?php echo e($column->id); ?><?php echo e($application->id); ?>"
                                                    ...
                                                    $currentDate < $application->schedule->schedule_date->timestamp): ?>
                                                        <button onclick="sendReminder(<?php echo e($application->id); ?>, 'reminder')"
                                                            type="button"
                                                            class="btn btn-sm btn-info notify-button"><?php echo app('translator')->get('app.reminder'); ?></button>
                                                    <?php endif; ?>
                                                    <?php if(in_array($column->status,  ['hired', 'rejected'])): ?>
                                                        <button onclick="sendReminder(<?php echo e($application->id); ?>, 'notify', '<?php echo e($column->status); ?>')" type="button" class="btn btn-sm btn-info notify-button"><?php echo app('translator')->get('app.notify'); ?></button>
                                                    <?php endif; ?>
                                                </span>
...
<script>
    ...
    function sendReminder(id, type, status) {
        ...
        if (type == 'notify') {
            if (status == 'hired') {
                msg = "<?php echo app('translator')->get('errors.sendHiredNotification'); ?>";
            } else {
                msg = "<?php echo app('translator')->get('errors.sendRejectedNotification'); ?>";
            }
        } else {
            msg = "<?php echo app('translator')->get('errors.sendInterviewReminder'); ?>";
        }
        swal({
    ...
    $(function () {
        ...
        $('.lobipanel').on('dragged.lobiPanel', function (e, lobiPanel) {
            ...
            if (oldParentId == 3 && currentParentId == 4) {
                ...
                var button = '<button onclick="sendReminder(' + currentApplicationId + ', \'notify\')" type="button" class="btn btn-sm btn-info notify-button"><?php echo app('translator')->get('app.notify'); ?></button>';
                ...
                if (currentDate < timeStamp) {
                    $('#buttonBox' + oldParentId + currentApplicationId).show();
                    var button = '<button onclick="sendReminder(' + currentApplicationId + ', \'reminder\')" type="button" class="btn btn-sm btn-info notify-button"><?php echo app('translator')->get('app.reminder'); ?></button>';
                    $('#buttonBox' + oldParentId + currentApplicationId).html(button);
                }
                $('#buttonBox' + oldParentId + currentApplicationId).attr('id', 'buttonBox' + currentParentId + currentApplicationId);
            } else {
...
</script><?php /**PATH C:\laragon\www\loker-apps-new\resources\views/admin/job-applications/board-data.blade.php ENDPATH**/ ?>